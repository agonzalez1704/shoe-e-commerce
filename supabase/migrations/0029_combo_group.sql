-- Combo pool: products sharing a non-null combo_group form ONE combo pool.
-- Any combo_min_qty units across the pool (mix models) cost combo_price_cents
-- per group. Replaces the per-product-only combo. (List: Frosinone/Acropolis/
-- Londres = "trio", 2 x $1,999.)
alter table products add column if not exists combo_group text;
create index if not exists products_combo_group_idx on products (combo_group) where combo_group is not null;

-- Replace create_order combo math with the pooled (cross-model) version.
