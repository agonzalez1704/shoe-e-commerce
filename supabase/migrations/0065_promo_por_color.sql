-- Promociones por color. Una promocion puede cubrir solo algunos colores de un
-- modelo (p. ej. Roma negro/rojo, grabado lizard): promocion_productos.colores
-- lista los colores cubiertos; null = todos (lo de antes). create_order y
-- promo_percent lo respetan; el catalogo y el carrito lo espejan en TS.

alter table promocion_productos add column if not exists colores text[];

create or replace function public.promo_percent(p_product_id uuid, p_color text default null)
returns int language sql stable security definer set search_path = public as $$
  ...
  where pp.product_id = p_product_id
    and (pp.colores is null or p_color = any(pp.colores))
    and pr.active
    and pr.starts_at <= now()
    and pr.ends_at > now();
$$;
drop function if exists public.promo_percent(uuid);
grant execute on function public.promo_percent(uuid, text) to anon, authenticated;

-- create_order: 0064 verbatim salvo el filtro de color en las dos busquedas de promo.
