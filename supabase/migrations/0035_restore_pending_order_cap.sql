-- Restore the abuse cap that 0011 added but 0026/0029/0031 dropped when they
-- recreated create_order for combo pricing / the discount-alias fix.
--
-- Without it a guest can spam unpaid cash orders that each reserve stock for up
-- to 3 days (inventory exhaustion). Cap concurrent unpaid orders per identity:
--   logged-in -> per customer_id
--   guest     -> per cart session_token (stamped onto the order)
-- Also re-stamp orders.session_token so the guest branch can match.
--
-- Body is 0031 verbatim + the cap block + session_token; the pending count now
-- also covers NULL-expiry orders (aplazo / mercadopago redirects), which the
-- original 0011 count missed.
