-- Prefijo del número de pedido, por tienda.
--
-- `create_order` lo tenía escrito a mano como 'BL-', así que una segunda tienda
-- entregaba a sus compradores un folio con las iniciales de la primera. Pasa a
-- la tabla settings, que ya existe para este tipo de valor.
--
-- Sin fila el prefijo sigue siendo 'BL-': Blade no cambia de comportamiento y
-- esta migración no le toca un solo pedido. La tienda que quiera otro folio
-- inserta su fila; no hay despliegue de por medio.
--
-- Los pedidos ya emitidos NO se renumeran. Un folio es lo que el comprador
-- tiene en su correo y lo que teclea en /rastrear; reescribirlo rompería las
-- dos cosas. La búsqueda es por coincidencia exacta, así que los BL- viejos y
-- los nuevos conviven sin problema.
--
-- El cuerpo de la función es idéntico al de 0037 salvo esas dos líneas: la
-- firma y las columnas de retorno no se tocan, que es lo que rompería el
