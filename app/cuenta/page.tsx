                  ...
                  {o.status === "pending" ? (
                    ...
                    <>
                      ...
                      {(() => {
                        ...
                        const filas = (o as unknown as { order_items_combo: { quantity: number; variants: { fuera_de_combo: boolean; products: { combo_group: string | null; combo_min_qty: number | null } | null } | null }[] }).order_items_combo ?? [];
                        const g = new Map<string, { min: number; u: number }>();
                        for (const f of filas) {
                          const pr = f.variants?.products;
                          if (f.variants?.fuera_de_combo || !pr?.combo_group || pr.combo_min_qty == null) continue;
